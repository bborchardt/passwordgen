...
// pw.js expects sjcl as a global, the way the browser provides it.
global.sjcl = require("../web/js/sjcl.js");
...
test("v1 passwords are unchanged", async (t) => {
    for (const [input, length, expected] of golden.vectors) {
        await t.test(`getPassword(${JSON.stringify(input)}, ${length})`, () => {
            assert.strictEqual(pw.getPassword(input, length), expected);
        });
    }
});
...
test("v1 raw hashes are unchanged", async (t) => {
    for (const [input, length, expected] of golden.hashes) {
        await t.test(`getEncodedHash(${JSON.stringify(input)}, ${length})`, () => {
            assert.strictEqual(pw.getEncodedHash(input, length), expected);
        });
    }
});
...
test("returns exactly the requested number of characters", () => {
    ...
    for (const length of [12, 100, 200, 250, 256]) {
        assert.strictEqual(pw.getPassword("correct horse battery staple", length).length, length);
    }
});

test("emits only unreserved alphanumerics", () => {
    for (const length of [12, 64, 200]) {
        assert.match(pw.getPassword("a passphrase", length), /^[A-Za-z0-9]+$/);
    }
});
...
test("retries deterministically rather than recursing without bound", () => {
    // "passphrase1" fails the mix check on its first derivation, so this
    // exercises the retry path; it must be stable across calls.
    const first = pw.getPassword("passphrase1", 12);
    assert.strictEqual(pw.getPassword("passphrase1", 12), first);
